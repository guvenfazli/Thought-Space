...
export default function UserProfile({ userId }) {
  ...
  return (
    <div className="w-full">
      {userLoading ? <p>User is Loading</p> :
        <div className="mb-14">
          ...
          <div className="flex flex-col gap-y-4 mb-4">
            ...
            {postsLoading ? <p>Posts are loading...</p> : postValues.map((post) =>
              <div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -5, transition: { duration: 0.5 } }} className="border-y-2 py-3 px-2 flex flex-col gap-y-2" key={post.id}>
