    ...
    return (
      <div className={`${openNav ? "h-72" : "h-11"} duration-500 ease-in-out overflow-hidden hidden max-sm:flex`}>
        <nav className={`flex flex-col transition-all ${openNav && "justify-between"} relative ${openNav && "h-full"} items-center duration-150 ease-in-out overflow-hidden`}>
          ...
          <Link href={'/newTask'} className={`text-lg flex items-center justify-center ${!openNav && "absolute bottom-1 invisible"} opacity-100 font-bold h-10 w-10 duration-150 bg-gray-900 px-3 py-1 rounded-full hover:bg-gray-500`}>+</Link>
          <Link href={`/available/users/${userId?.id}`} className={`text-lg flex items-center justify-center ${!openNav && "absolute bottom-1 invisible"} opacity-100 font-bold h-10 w-10 duration-150  bg-gray-900 px-3 py-1 rounded-full hover:bg-gray-500`}>{profile}</Link>
          <Link href={'/available'} className={`text-lg flex items-center justify-center ${!openNav && "absolute bottom-1 invisible"} opacity-100 font-bold h-10 w-10 duration-150 bg-gray-900 px-3 py-1 rounded-full hover:bg-gray-500`}>{home}</Link>
          <button onClick={() => setOpenNav(prev => !prev)} className={`text-lg 
          font-bold 
          duration-150
          ease-in-out 
          bg-gray-900 
          ${!openNav && "w-full h-full"}
          h-10
          w-10
          rounded-full
          hover:bg-gray-500 
